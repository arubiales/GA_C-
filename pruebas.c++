...
int fit(double max_journey, double food_price, int people, int pop, int gen, float p_m, float p_c, int k);
vector <int> create_first_route();
...
int main(){
    read_csv("datos_ga.csv");

    vector<int> route{10000002, 70,71,72,86,92,97,99}, first_route = id_origin;
    float coste = 0;

    // first_route = create_first_route();

    set<int> s(first_route.begin(), first_route.end());


    first_route.erase(first_route.begin(), first_route.end());

    first_route.assign(s.begin(), s.end());
    sort(first_route.begin(), first_route.end(), greater<>());

    for(auto i:first_route){
        cout<<i<<" ";
    }


    // show_vector_int(first_route);
...
// vector <int> create_first_route(){
//     vector<int> first_route = id_origin;

//     first_route.erase(unique(first_route.begin(), first_route.end(), first_route.end()));
//     return first_route;
// }
...
//     for(int i=0; i<pop; i++){
//         population.push_back(shuffle_vect(route));
//     }
